...
- **Models** — per-role tier. The shipped tier runs every role at the **strong** tier except `scout`, which defaults to `claude-sonnet-5` (bounded and external). Surveyor and inspector are judgment roles, so they stay strong. Offer it as the starting point; let them adjust any role. `inherit` = use the session model.
