...
- **Board** (`.crew/board.md`) — the foreman's **live orchestration state**. Write a title and a one-line note that this is the foreman's live snapshot — rewritten as work progresses, holding only what is in flight or pending, completed work dropping off; **not a log and not a source of truth** (git history, memory, and plan files are). Leave it otherwise empty. Record `board.path`; it takes no sha.
